...
science \fixme{REF:Chi} \fixme{REF: Webb}, conflict resolution in
socio-cognitive theories \fixme{REF:Doise},
mutual regulation \fixme{REF: Blaye} in a Vygostkian perspective, etc. The
...
REF\fxfatal{REF missing} reviews how the features of the collaborative
...
\M{B}{C}{X})$. If it is the same, $\mathcal{H}_{2}$ and
$\mathcal{H}_{3}$\fxfatal{why H3?} gain
over $\mathcal{H}_{1}$
...
mode'' and 24 individuals ``long time in scout mode''\fxfatal{Inconsistent! 36
participants in awareness condition??}. A two-way analysis of
...
significant as well: $r=0.43, p <.001$\fxfatal{Attention! $\Delta_2$ and
$\Delta_3$ seem to be inverted! Cf fig 2}.
...
ranked\fxfatal{How are they ranked?}. We used
...
\gModel{B}{A},...): The correlation is -0.22 \fxfatal{p value?}: teams with a good \gModel{A}{B}
...
area  (perceiving emotions)\fxfatal{Paragraph to rewrite}. 

\subsubsection*{Discussion}

\fixme{TBD}
\fixme{Check si il faut modifier les resultats de la symmetries}
...
cognitive attitude\fxfatal{to be better justified}.
...
between \gModel{A}{B} and the collaborative learning gain\fxfatal{What is the
collaborative learning gain} of the pair $\{A,
...
\textit{Dependent var.}   & Team/Ind. performance            & Team/Ind. performance           & Team performance          & Team/Ind. knowledge gains & Team/Ind. knowledge gains    \\
    \textit{Independent var.} & Awareness tool \textit{vs} not.           & Awareness tool \textit{vs}  not.          & Audio \textit{vs} Audio+Video & Script \textit{vs} not.            & Awareness tool \textit{vs} not.       \\
\textit{MM°-DV correlation}   & +0.42                            & +0.15                           & +0.22                     & +0.08                     & +0.40                        \\
\textit{$\Delta_1$}                   & +0.38 (significant)                      & +0.41 (significant)                     & ???                       & +0.26 (near significant)          & +0.05 (ns)              \\
...
and 3\fxfatal{No $\Delta_2$ in study 1! Dyads!} reinforce both $\mathcal{H}_{2}$ and $\mathcal{H}_{3}$. The
