...
$B$ have the same accuracy when modelling $C$ ($\Delta_2 =
\Delta(\mathcal{M}^{\circ}(A,C,X),
\mathcal{M}^{\circ}(B,C,X))$)? If it is the same, $\mathcal{H}_{2}$ gains over
$\mathcal{H}_{1}$

Conversely, does $C$ model more accurately $A$ than $B$ ($\Delta_3=
\Delta(\mathcal{M}^{\circ}(C,A,X),
\mathcal{M}^{\circ}(C,B,X))$)? A positive answer would support $\mathcal{H}_{3}$.
