...
\subsection{Related research of the applicants}


Pierre Dillenbourg has been working since several years on the role and importance
of mutual modelling in human-human interaction, specifically in the context of
learning sciences.

He already conducted several experiments~\cite{sangin2007partner,
nova2007collaboration} that supported the idea that modelling partners is not only
an individual skill. When engaged in a social task that could benefit from
mutual modelling, the accuracy of the models build by each of the participant on
the other participants increase.

In the frame of this work, Pierre Dillenbourg has proposed a simple formalism
that we describe below, and that we will use subsequently to describe the mutual
modelling situations.

\paragraph{A simple notational system for mutual modeling} We suggest
the notation \model{A}{B}{X} to denote ``A knows that B knows X". This is a
simple but hopefully useful reduction to reason on mutual modeling. This
...
To estimate the mutual modelling effort, we need 3 variables:
